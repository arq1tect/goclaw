...
func builtinToolSeedData() []store.BuiltinToolDef {
	defs := []store.BuiltinToolDef{
		...
		{Name: "agent_grants", DisplayName: "Agent Grants", Description: "Manage skill and MCP-server grants per agent. Actions: list (granted for agent), grant_skill / revoke_skill (with version), grant_mcp / revoke_mcp (with tool_allow/deny/config_overrides), list_skills_available / list_mcp_available (catalog enumeration). IDs accept UUID or slug/name. Tenant-scoped. Admin-class.", Category: "admin", Enabled: false},
		{Name: "agent_hooks", DisplayName: "Agent Hooks", Description: "Manage agent and tenant scope hooks. Actions: list (filtered), get, create, update (partial patch), delete, toggle (enable/disable shortcut), set_agents (replace agent assignment). Events: session_start, user_prompt_submit, pre/post_tool_use, stop, subagent_start/stop. Handlers: command, http, prompt (LLM-validation), script. Global scope is blocked (requires master-scope). Tenant-scoped. Admin-class.", Category: "admin", Enabled: false},
		{Name: "agent_telegram", DisplayName: "Agent Telegram", Description: "Manage Telegram channels for agents via Managed Bots flow (Bot API 9.6). Actions: list_channels, request_managed_bot (sends deep-link to operator), poll_managed_bot_token (polls Telegram for child token, registers as new channel), unlink. Polling-based — no channels-layer extension needed. Tenant-scoped. Admin-class.", Category: "admin", Enabled: false},
