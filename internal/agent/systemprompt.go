...
var coreToolSummaries = map[string]string{
	...
	"knowledge_graph_search": "Search the knowledge graph for entities (people, companies, jurisdictions, contracts, banks, etc.) and how they connect — use for multi-hop relationship questions or to discover what's in your agent's catalog (preset-dependent)",
	"knowledge_graph_mutate": "Create or update entities and relations in the knowledge graph. Search first to avoid duplicates. Use types from your agent's catalog (preset-dependent); to introduce a type not in the catalog, activate skill `kg-ontology` for the proposal protocol — do not force a wrong fit",
