...
// via scopeClause.
func (s *SQLiteAgentStore) DeleteAgentContextFile(ctx context.Context, agentID uuid.UUID, fileName string) (bool, error) {
	tClause, tArgs, err := scopeClause(ctx)
	if err != nil {
		return false, err
	}
	res, err := s.db.ExecContext(ctx,
		"DELETE FROM agent_context_files WHERE agent_id = ? AND file_name = ?"+tClause,
		append([]any{agentID, fileName}, tArgs...)...,
	)
	if err != nil {
		return false, err
